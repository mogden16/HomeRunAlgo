"""Run the full daily ET live refresh: train through yesterday, settle, then publish today."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--feature-profile",
        default="pregame_safe_v1",
        choices=["stable", "pregame_safe_v1", "live", "live_plus", "live_shrunk", "live_shrunk_precise", "live_usable_candidate_v1", "live_usable_candidate_v2", "live_usable_candidate_v3", "live_xstats_v1", "live_bat_tracking_v1", "expanded", "all"],
...
def run_daily_live_refresh(
    *,
    ...
    print("\n[3/4] Publishing today's slate")
    try:
        picks = publish_live_picks(
            dataset_path=dataset_path,
            bundle_path=bundle_path,
            metadata_path=metadata_path,
        ...
        raise RuntimeError(
            "Daily live refresh failed during publish after refresh/train/settle. "
            f"publish_date={resolved_publish_date}"
        ) from exc

    print("\n[4/4] Daily workflow data updates complete")
    print(f"Published pick count       : {len(picks)}")
    return picks


def main() -> None:
    args = parse_args()
    run_daily_live_refresh(
