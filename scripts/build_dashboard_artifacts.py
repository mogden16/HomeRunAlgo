...
def current_pick_sort_key(row: dict[str, Any]) -> tuple[float, int, str]:
    overlay_rank = parse_int(row.get("ballparkpal_overlay_adjusted_rank"))
    return (
        float(str(row.get("game_date") or "").replace("-", "") or 0),
        int(overlay_rank or row.get("original_rank") or row.get("rank") or 999),
...
def normalize_pick(row: dict[str, Any], tracking_start_date: str) -> dict[str, Any] | None:
    ...
    normalized = {
        ...
        "ballparkpal_overlay_adjusted_rank": parse_int(row.get("ballparkpal_overlay_adjusted_rank")),
